...
import { isWithin, arkToolkitStateRoot } from "./paths.js";
 ...
 */
export async function prepareArtifactAccessKey(root = arkToolkitStateRoot()) {
...
function artifactRoot(path) {
    ...
    while (true) {
        if (basename(current) === 'artifacts' && basename(dirname(current)) === '.dsh-ark-toolkit')
            return current;
