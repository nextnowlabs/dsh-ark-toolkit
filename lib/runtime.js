...
export class VisionToolkitRuntime {
    ...
    async generateImage(request, options) {
        return this.runOperation('vision_generate_image', options, async (operation) => {
            ...
            if (prompt.length === 0)
                throw new VisionToolkitError('input', 'generate_image: prompt must not be empty');
            ...
            try {
                ...
                if (operation.signal.aborted)
                    throw new VisionToolkitError('cancelled', 'vision_generate_image: cancelled');
    ...
    async speak(request, options) {
        return this.runOperation('vision_speak', options, async (operation) => {
            ...
            if (text.length === 0)
                throw new VisionToolkitError('input', 'speak: text must not be empty');
            if (text.length > 2000)
                throw new VisionToolkitError('input', 'speak: text must not exceed 2000 characters');
            ...
            if (resolved === undefined) {
                throw new VisionToolkitError('config', `credential ${tts.credential} is not configured; set the Volcengine TTS key through DSH credentials`);
            }
            operation.metrics.usedVisionService = true;
            const audio = {
            ...
            try {
                ...
                if (operation.signal.aborted)
                    throw new VisionToolkitError('cancelled', 'vision_speak: cancelled');
            ...
            if (response.body === null)
                throw new VisionToolkitError('runtime', 'speak: Volcengine TTS returned no body');
            const chunks = [];
            let format = encoding;
            let sawAudio = false;
                    ...
                    while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
                        ...
                        if (typeof event.audio === 'string') {
                            ...
                            if (total > MAX_SPEECH_BYTES)
                                throw new VisionToolkitError('capacity', 'speak: synthesized audio exceeds the 64 MiB limit');
                        }
            ...
            if (!sawAudio)
                throw new VisionToolkitError('output', 'speak: Volcengine TTS returned no audio');
