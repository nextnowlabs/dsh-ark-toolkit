    ...
    async saveCredential(request) {
        ...
        const target = request.ref === arkRef ? arkRef
            : request.ref === ttsRef ? ttsRef
                : undefined;
        if (target === undefined) {
            throw new CredentialReferenceConflictError(`credential reference "${request.ref}" does not match the configured "${arkRef}" or "${ttsRef}"; reload Settings and try again`);
        }
