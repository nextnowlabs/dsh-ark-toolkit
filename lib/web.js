...
import { resolveConfig, ARK_TOOLKIT_SETTINGS_NAMESPACE, } from "./config.js";
...
function descriptorOf(ctx) {
    ...
    if (descriptor === undefined)
        throw new Error('ark-toolkit Settings namespace is not registered');
    return descriptor;
}
...
export class ArkToolkitWebBackend {
    ...
    async save(request) {
        ...
        await this.ctx.settings.replace(ARK_TOOLKIT_SETTINGS_NAMESPACE, request.value, request.expectedRevision);
    ...
    async saveCredential(request) {
        ...
        if (descriptor.revision !== request.expectedRevision) {
            throw new SettingsConflictError(ARK_TOOLKIT_SETTINGS_NAMESPACE, request.expectedRevision, descriptor.revision);
        }
        ...
        const target = request.ref === visionRef ? visionRef
            : request.ref === ttsRef ? ttsRef
                : undefined;
        if (target === undefined) {
            throw new CredentialReferenceConflictError(`credential reference "${request.ref}" does not match the configured "${visionRef}" or "${ttsRef}"; reload Settings and try again`);
        }
 ...
 */
export function installArkToolkitWeb(ctx, backend, artifacts, pastedImages, pastePolicy, getDisplayConfig) {
