...
export async function cropRegionToDataUrl(path, region) {
    let buffer;
    try {
...
export async function imageToDataUrl(path) {
    let bytes;
    try {
    ...
    const format = await probeImage(path).then(info => info.format).catch(() => 'png');
 ...
 */
export async function compressImage(sourcePath, destPath, maxBytes, maxPixels) {
    ...
    const minEdge = 64;
    const maxSteps = 4;
    const fits = (w, h, bytes) => w >= 1 && h >= 1 && w * h <= maxPixels && bytes <= maxBytes;
    let best;
    const saveCandidate = async (pipeline, lossy, resized, candidate) => {
        let buffer;
        try {
    ...
    for (let step = 0; step <= maxSteps; step += 1) {
        ...
        if (step >= maxSteps || Math.min(width, height) <= minEdge)
            break;
        // Downscale based on how far the best candidate is over budget.
        ...
        const byteRatio = best !== undefined && best.bytes > maxBytes
            ...
            : 1;
        const ratio = Math.max(0.6, Math.min(0.95, Math.max(pixelRatio, byteRatio)));
