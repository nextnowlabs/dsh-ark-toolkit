...
function openaiMessageText(value) {
    if (!isRecord(value))
        return '';
...
function anthropicMessageText(value) {
    if (!isRecord(value))
        return '';
...
function prependLanguage(text, language) {
 ...
 */
export function buildGlancePrompt(query, ocr, imageCount) {
    const scope = imageCount > 1 ? 'these images' : 'this image';
    if (ocr) {
        return (`Transcribe every piece of visible text in ${scope} verbatim (titles, body text, labels, watermarks, etc.), `
 ...
 */
export async function describeImages(dataUrls, prompt, options) {
    if (dataUrls.length === 0)
        throw new ArkToolkitError('input', 'vision_api: at least one image is required');
    ...
    try {
        ...
        if (options.signal.aborted)
            throw new ArkToolkitError('cancelled', 'ark_glance: cancelled');
        throw new ArkToolkitError('runtime', `ark_glance: request failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
            throw new ArkToolkitError('service', `ark_glance: ${detail}; verify the configured credential`);
        }
        if (response.status === 429) {
            throw new ArkToolkitError('service', `ark_glance: ${detail}; retry later or reduce concurrency`);
        }
        if (response.status >= 500 || response.status === 408) {
            throw new ArkToolkitError('service', `ark_glance: ${detail}`);
        }
        throw new ArkToolkitError('service', `ark_glance: ${detail}`);
    }
    let body;
    try {
        ...
        throw new ArkToolkitError('output', 'ark_glance: vision API returned a non-JSON response', { cause: error });
    }
    ...
    if (answer.trim().length === 0) {
        throw new ArkToolkitError('output', 'ark_glance: vision API returned an empty description');
    }
    return { text: answer.trim(), upstreamMs };
}
//# sourceMappingURL=vision-api.js.map