...
export function arkToolkitStateRoot() {
...
export async function resolveAuthorizedFile(raw, policy, extensions, kind) {
    ...
    if (!info.isFile())
        throw new ArkToolkitError('input', `${kind} is not a regular file: ${raw}`);
    const extension = real.slice(real.lastIndexOf('.')).toLowerCase();
    if (!extensions.includes(extension)) {
        throw new ArkToolkitError('input', `unsupported ${kind} format "${extension || '(none)'}"; supported: ${extensions.join(', ')}`);
    }
...
export function resolveOutputFile(raw, policy, defaultName, extensions) {
    ...
    if (isAbsolute(expanded))
        throw new ArkToolkitError('path', 'output must be a filename, not an absolute path');
...
export function resolveOutputDirectory(raw, policy, defaultName) {
    ...
    const expanded = expandUserHome(name);
    if (isAbsolute(expanded))
        throw new ArkToolkitError('path', 'artifact directory must not be an absolute path');
    const segments = expanded.split(/[\\/]/);
...
export async function commitStagedOutput(staged, finalPath, policy) {
    ...
    if (!info.isFile())
        throw new ArkToolkitError('output', 'upstream output is not a regular file');
    try {
