...
async function readImageBlock(ctx, runtime, block, query, sessionId) {
    ...
    try {
        ...
        if (answer.length === 0)
            throw new Error('the Ark Toolkit returned an empty description');
...
export async function labelTakeoverVerdict(ctx, label) {
    ...
    for (const provider of llm.listProviders()) {
        ...
        try {
            ...
            ctx.logger.warn('dsh-ark-toolkit: paste verdict could not read route "%s"; native paste wins for this label. %s', provider.id, messageOf(error).slice(0, 300));
            return false;
        }
...
export function installImageInputVariants(ctx, getConfig, getRuntime) {
    ...
    const release = (upstream) => {
        ...
        try {
            ...
            ctx.logger.warn('dsh-ark-toolkit: image-input variant release failed for "%s": %s', upstream, messageOf(error));
        }
    };
    const releaseAll = () => {
        stale.clear();
        ...
        try {
            ...
            for (const provider of providers) {
                ...
                try {
                    ...
                    ctx.logger.warn('dsh-ark-toolkit: image-input variant registration skipped for "%s": %s', upstream, messageOf(error));
                }
            }
        }
        catch (error) {
