...
export async function apply(ctx, config = {}) {
    ...
    try {
        ...
        ctx.logger.error('dsh-ark-toolkit %s: runtime not ready; the vision-skills skill, activation bootstrap, and Agent-scoped visual tools are NOT registered. Settings remain available for repair. %s', PLUGIN_VERSION, message);
    }
    ...
    installArkToolkitWeb(ctx, backend, artifacts, pastedImages, createPasteTakeoverResolver(ctx, () => resolveConfig(settings.get())), () => ({ hidden: resolveConfig(settings.get()).imageInputVariants.hidden }));
