...
| "Answer this question about the image" | `vision_glance` |
| "Transcribe the text in this image / screenshot" | `vision_glance` (`ocr: true`) |
| "What changed between these two images?" | `vision_glance` (pass both images together) |
| "Create an image from this description" | `vision_generate_image` |
| "Read this text out loud" | `vision_speak` |

`vision_glance` sends the image to the configured vision service and returns
...
## vision_generate_image — create an image with Seedream
...
## vision_speak — synthesize speech with ByteDance TTS
...
reason past a missing detail. Look again yourself: call `vision_glance` with
